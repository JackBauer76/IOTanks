...
void turnback()                   //Function
{
  ...
  //motor1.run(FORWARD);    // Originally here, but now I am trying inside the for loop below
  //motor2.run(BACKWARD);
  int counterBlue;
...
void obstacleFwd() {              //Function with driving logic
  ...
  else if (distanceObstacleFwd < 50)      // far obstacle forward
    {
      Serial.println("Obstacle near ahead");  
      turnright();
     }                 }

void obstacleBwd() {                       //Function with driving logic, has to be solved in some other way
  int distanceObstacleBwd = distance2driveBwd();
  if (distanceObstacleBwd == 0)
    {   delay(5);    }
  else if (distanceObstacleBwd < 15)     // close obstacle backward
      {
