...
void turnleft()               //Function
{
  ...
  int counterGreen;
  int randomTime = random(250,1000);
  for(int counterGreen = 0; counterGreen < randomTime; counterGreen++)
    {
          motor2.run(FORWARD);
          motor1.run(BACKWARD);
...
void turnright()              //Function
{
  ...
  int counterRed;
  int randomTime = random(250,1000);
  for(int counterRed = 0; counterRed < randomTime; counterRed++)
    {
          motor1.run(FORWARD);
          motor2.run(BACKWARD);
...
void obstacleFwd() {              //Function with driving logic
  ...
  if (distanceObstacleFwd == 0)
    {   delay(10);    }
  else if (distanceObstacleFwd < 30)      // close obstacle forward
    {
      Serial.println("Obstacle ahead");
      stopvehicle();
      while(distanceObstacleFwd <secureDistance )
      {
       turnback();
       Ultrasonic ultrasonicFwd(trigPinFwd, echoPinFwd);
       int distanceObstacleFwd = ultrasonicFwd.distanceRead();
       ...
       Serial.println(distanceObstacleFwd);
       if (distanceObstacleFwd > secureDistance)
       {
...
void obstacleBwd() {              //Function with driving logic
  Ultrasonic ultrasonicBwd(trigPinBwd, echoPinBwd);  
  int distanceObstacleBwd = distance2driveBwd();
  if (distanceObstacleBwd == 0)
    {   delay(10);    }
  else if (distanceObstacleBwd < 10)      // close obstacle forward
    {
      Serial.println("Blockage behind");
      stopvehicle();   
      halfSpeed();
      motor1.run(FORWARD);
      motor2.run(FORWARD);
