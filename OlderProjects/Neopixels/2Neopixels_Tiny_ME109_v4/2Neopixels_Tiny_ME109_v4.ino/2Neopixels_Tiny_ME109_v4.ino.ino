...
void lightingPattern1()
{
    ...
    theaterChase(pixels.Color(127, 0, 0), 20); // Blue
    //theaterChase(guns.Color(127, 0, 127), 20); // Blue
}

void lightingPattern2()
{
    colorWipe(pixels.Color(150, 0, 0), 20); // Blue
}
...
void theaterChase(uint32_t c, uint8_t wait) 
{
  ...
  {  //do 10 cycles of chasing
    for (int q=0; q < 3; q++) 
    {
      ...
      pixels.show();
      tone(Buzzer, 1000, 15);  // new line inserted
      delay(5);   // new line inserted
      noTone(Buzzer); // new line inserted
      delay(wait-5);
      for (uint16_t i=0; i < pixels.numPixels(); i=i+3) 
      {
